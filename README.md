...
Trained on daily air quality observations with a **strict time-based split** (trained on earlier dates up to May 2024; evaluated on held-out dates through September 2026) to prevent lookahead data leakage:
...
> **Key Model Finding:** The XGBoost model achieved an $R^2$ of **0.9817** on held-out test data, with a Mean Absolute Error of just 8.09 AQI points across Delhi, Mumbai, and Bengaluru.
...
- **Historical Date Lookup**: Browse historical days across Indian metros (with data through 2026) and compare model predictions against ground truth measurements.
...
- **Plain-English Policy Briefing**: Synthesized news-style advisory generated by Google Gemini (with an automatic offline domain-expert fallback).
